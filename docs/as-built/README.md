...
| [65-coder-agents.md](65-coder-agents.md) | Coder Agents control-plane chat: the curated 4-model picker (reasoning effort + estimated cost), the read-only datastore MCP server (supported path; gateway-injected MCP removed), and chat spend-limits (tiered caps, HTTP 409 enforcement). GitLab MCP was evaluated and dropped (CODAGT-570). |
