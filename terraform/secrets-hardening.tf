...
resource "aws_iam_role" "external_secrets" {
  ...
  description        = "External Secrets Operator: read usgov-coderdemo/* from Secrets Manager (IRSA)"
}
...
data "aws_iam_policy_document" "external_secrets" {
  statement {
    ...
    resources = [
      "arn:${data.aws_partition.current.partition}:secretsmanager:${var.region}:${data.aws_caller_identity.current.account_id}:secret:usgov-coderdemo/*",
    ]
  }
}
...
#   aws eks associate-encryption-config --cluster-name usgov-coderdemo \
