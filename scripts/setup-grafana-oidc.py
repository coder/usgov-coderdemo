...
What it does:
  1. Create/update a confidential OIDC client `grafana` (standard flow, PKCE
     S256, redirect URI https://grafana.usgov.coderdemo.io/login/generic_oauth).
  ...
  3. Read the client secret and upsert it into AWS Secrets Manager at
     usgov-coderdemo/observability/grafana-oauth as {"client-secret": "..."}.
...
REALM = "coder"
CLIENT_ID = "grafana"
GRAFANA_URL = "https://grafana.usgov.coderdemo.io"
REGION = "us-gov-west-1"
ASM_NAME = "usgov-coderdemo/observability/grafana-oauth"
    ...
    try:
        ...
        subprocess.run(
            ["aws", "secretsmanager", "create-secret", "--region", REGION,
             "--name", name,
             "--description", "usgov-coderdemo Grafana OIDC client secret (ESO).",
             "--secret-string", ref],
            check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        return "created"
    finally:
