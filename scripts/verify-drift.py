#!/usr/bin/env python3
"""
verify-drift.py - read-only drift diagnostic for the usgov-coderdemo GovCloud
...
~/.config/usgov-coderdemo/generated-secrets.env and used only to obtain a
...
Usage (from the repo root, with the demo kubeconfig + env):
    set -a; . ~/.config/usgov-coderdemo/env; \\
        . ~/.config/usgov-coderdemo/generated-secrets.env; set +a
...
# in ASM secret usgov-coderdemo/coder/ai under the same JSON field.
ANTHROPIC_NS = "coder"
