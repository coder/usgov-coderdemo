...
What it does:
  1. Create/update a confidential OIDC client `kiali` (standard authorization-code
     flow, PKCE S256, redirect URIs https://kiali.usgov.coderdemo.io/kiali/* and
     the bare https://kiali.usgov.coderdemo.io/kiali). Kiali's callback lives
  ...
  3. Read the client secret and upsert it into AWS Secrets Manager at
     usgov-coderdemo/observability/kiali-oauth as {"oidc-secret": "..."}.

The ASM JSON key is intentionally `oidc-secret` (not `client-secret` as for
grafana): Kiali v2.26 reads the OIDC client secret from a Secret named `kiali`
(ns istio-system) under the key `oidc-secret`. The kiali-oauth ExternalSecret
uses dataFrom.extract, so the ASM JSON key becomes the Kubernetes Secret key
verbatim, producing exactly the `oidc-secret` key Kiali expects.

Reads admin credentials from ~/.config/usgov-coderdemo/generated-secrets.env:
...
REALM = "coder"
CLIENT_ID = "kiali"
KIALI_URL = "https://kiali.usgov.coderdemo.io"
REGION = "us-gov-west-1"
ASM_NAME = "usgov-coderdemo/observability/kiali-oauth"
    ...
    try:
        ...
        subprocess.run(
            ["aws", "secretsmanager", "create-secret", "--region", REGION,
             "--name", name,
             "--description", "usgov-coderdemo Kiali OIDC client secret (ESO).",
             "--secret-string", ref],
            check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        return "created"
    finally:
...
def main():
    ...
    action = put_asm(ASM_NAME, {"oidc-secret": secret})
