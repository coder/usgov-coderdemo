...
Secrets Manager under the usgov-coderdemo/* prefix, so External Secrets Operator
...
MAPPING = {
    "usgov-coderdemo/coder/db": ("coder", "coder-db"),
    "usgov-coderdemo/coder/oidc": ("coder", "coder-oidc"),
    "usgov-coderdemo/coder/ai": ("coder", "coder-ai"),
    "usgov-coderdemo/coder/external-auth": ("coder", "coder-external-auth"),
    "usgov-coderdemo/coder/provisioner-alpha": ("coder", "coder-provisioner-alpha"),
    "usgov-coderdemo/coder/provisioner-bravo": ("coder", "coder-provisioner-bravo"),
    "usgov-coderdemo/keycloak/admin": ("keycloak", "keycloak-admin"),
    "usgov-coderdemo/keycloak/db": ("keycloak", "keycloak-db"),
    "usgov-coderdemo/gitlab/secrets": ("gitlab", "gitlab-secrets"),
}
    ...
    try:
        ...
        if asm_exists(name):
            ...
            sh(["aws", "secretsmanager", "create-secret", "--region", REGION,
                "--name", name,
                "--description", "usgov-coderdemo demo secret (synced to k8s by ESO)",
                "--secret-string", ref])
            return "created"
    finally:
        os.unlink(path)
