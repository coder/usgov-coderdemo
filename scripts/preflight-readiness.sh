#!/usr/bin/env bash
# preflight-readiness.sh — verify usgov-coderdemo workspace prerequisites (human pre-orchestrator).
# Usage: preflight-readiness.sh [--clone] [--env-file PATH] [--workspace-root PATH]
set -euo pipefail

CLONE=0
ENV_FILE="${HOME}/.config/usgov-coderdemo/env"
WORKSPACE_ROOT=""
...
Verify usgov-coderdemo workspace prerequisites before launching the orchestrator.

Options:
  --clone              git clone missing reference repos into REFERENCE_ROOT
  --env-file PATH      env file (default: ~/.config/usgov-coderdemo/env)
  --workspace-root DIR parent of usgov-coderdemo + reference (auto-detect if omitted)
  -h, --help           show help
EOF
}
...
detect_workspace_root() {
  ...
  if [[ -d "$dir/reference" ]] || [[ -d "$dir/usgov-coderdemo" ]]; then
    echo "$dir"
    return
  fi
  if [[ -d "$(dirname "$dir")/reference" ]] || [[ -d "$(dirname "$dir")/usgov-coderdemo" ]]; then
    echo "$(dirname "$dir")"
    return
  fi
  echo "$PWD"
...
echo "== usgov-coderdemo preflight readiness =="
...
if [[ -f "$ENV_FILE" ]]; then
  ...
  DEMOENV_ROOT="$(expand_home "${DEMOENV_ROOT:-usgov-coderdemo}")"
else
  bad "env file missing: $ENV_FILE (see docs/PRE-REQUISITES.md)"
  REFERENCE_ROOT="$WORKSPACE_ROOT/reference"
  DEMOENV_ROOT="$WORKSPACE_ROOT/usgov-coderdemo"
fi
...
DEMOENV_ROOT="${DEMOENV_ROOT:-usgov-coderdemo}"
...
if [[ -d "$DEMOENV_ROOT" ]]; then
  ...
  note "target repo not found: $DEMOENV_ROOT (git clone git@github.com:coder/usgov-coderdemo.git)"
fi
...
if [[ -f "$DEMOENV_ROOT/docs/AGENT-PRD.md" ]]; then
  ...
  note "doc pack not in $DEMOENV_ROOT/docs — clone github.com/coder/usgov-coderdemo"
fi
