...
When run with no flags it performs, in order:
  1. Create/update a confidential OIDC client `envdocs` in realm `coder`
     (standard flow, PKCE S256, redirect https://envdocs.usgov.coderdemo.io/
  ...
  2. Read the client secret, preserve (or generate once) the oauth2-proxy
     cookie secret, and upsert ASM usgov-coderdemo/envdocs/oauth as
  ...
  4. Generate the envdocs-site ConfigMap from docs/envdocs/ and apply the
     ...
     Services, Ingresses).
  5. Upsert a Route53 ALIAS envdocs.usgov.coderdemo.io -> the ingress-nginx NLB
     (more specific than the *.usgov.coderdemo.io wildcard, which points at the
...
REALM = "coder"
CLIENT_ID = "envdocs"
ENVDOCS_URL = "https://envdocs.usgov.coderdemo.io"
REGION = "us-gov-west-1"
ASM_NAME = "usgov-coderdemo/envdocs/oauth"
ROUTE53_ZONE_ID = "Z06701704WFETYIRU5C8"
INGRESS_NGINX_NS = "ingress-nginx"
INGRESS_NGINX_SVC = "ingress-nginx-controller"
ENVDOCS_FQDN = "envdocs.usgov.coderdemo.io"
DEFAULT_ACCOUNT_ID = "430737322961"
    ...
    try:
        ...
        subprocess.run(
            ["aws", "secretsmanager", "create-secret", "--region", REGION,
             "--name", name,
             "--description", "usgov-coderdemo envdocs OIDC client + oauth2-proxy cookie secret (ESO).",
             "--secret-string", ref],
            check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        return "created"
    finally:
...
def main():
    ...
    print("  curl -sS -o /dev/null -w '%{http_code} %{redirect_url}\\n' "
          f"https://{ENVDOCS_FQDN}/   # expect 302 -> auth.usgov.coderdemo.io")
