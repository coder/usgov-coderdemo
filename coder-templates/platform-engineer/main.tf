# =============================================================================
# Platform/DevOps engineer workspace (GovCloud demo, EKS, plain compute)
# =============================================================================
# A DevOps and platform-engineering workspace that runs in a Kubernetes pod on
# the EKS cluster. This is plain compute for the Coder Agents path: the AI runs
# server-side on the control plane, so this template carries NO AI Gateway
# wiring, NO LLM keys, and NO agent harness. It provisions infrastructure
# tooling (AWS CLI, kubectl, Helm, Terraform, jq) and code-server, and requires
...
# platform tooling. It is the only workspace base mirrored today. A pre-baked
# platform image (kubectl/helm/terraform/awscli already installed, no startup
# downloads) is the recommended end state for a fully air-gapped demo; see the
...
resource "coder_agent" "main" {
  ...
  # Best-effort platform toolchain provisioning. enterprise-base grants the
  # coder user passwordless sudo. Every step is tolerant so a transient failure
  # never fails the workspace build. awscli and jq come from apt; kubectl, helm,
  # and terraform are fetched from their official endpoints. In a fully
  # air-gapped boundary these downloads may be blocked: bake the binaries into a
  # custom workspace image (see WS-25 handoff) and this block becomes a no-op.
  startup_script = <<-EOT
    #!/bin/bash
    set +e
    touch ~/.bashrc
    ...
    if command -v apt-get >/dev/null 2>&1; then
      echo "=== Installing base platform tools (best-effort) ==="
      sudo apt-get update -y || true
      sudo apt-get install -y --no-install-recommends \
        awscli jq unzip ripgrep || true
    fi
    ...
    if ! command -v kubectl >/dev/null 2>&1; then
      echo "=== Installing kubectl (best-effort) ==="
      KVER="$(curl -sL https://dl.k8s.io/release/stable.txt 2>/dev/null)"
      if [ -n "$KVER" ]; then
        curl -sLo ~/.local/bin/kubectl "https://dl.k8s.io/release/$KVER/bin/linux/$GOARCH/kubectl" && \
          chmod +x ~/.local/bin/kubectl || true
      fi
    fi
    ...
    if ! command -v helm >/dev/null 2>&1; then
      ...
      curl -sSfL https://raw.githubusercontent.com/helm/helm/main/scripts/get-helm-3 | \
        HELM_INSTALL_DIR="$HOME/.local/bin" USE_SUDO=false bash || true
    fi
