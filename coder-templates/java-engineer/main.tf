# =============================================================================
# Java/JVM engineer workspace (GovCloud demo, EKS, plain compute)
# =============================================================================
# A well-tooled Java and JVM development workspace that runs in a Kubernetes pod
...
# wiring, NO LLM keys, and NO agent harness. It provisions a JVM toolchain
# (OpenJDK, Maven, Gradle) and code-server, and requires the in-boundary GitLab
# login for git.
#
...
# the JVM toolchain with apt. It is the only workspace base mirrored today. A
# pre-baked JDK image (no startup apt) is the recommended end state; see the
...
resource "coder_agent" "main" {
  ...
  # Best-effort JVM toolchain provisioning. enterprise-base grants the coder
  # user passwordless sudo. Every install is tolerant so a transient apt
  # failure never fails the workspace build. default-jdk on Ubuntu noble is
  # OpenJDK 21. For a fully air-gapped demo, bake these into a custom workspace
  # image (see WS-25 handoff) and the apt block becomes a no-op.
  startup_script = <<-EOT
    #!/bin/bash
    set +e
    touch ~/.bashrc
    ...
    if command -v apt-get >/dev/null 2>&1; then
      echo "=== Installing JVM toolchain (best-effort) ==="
