# =============================================================================
# Data scientist workspace (GovCloud demo, EKS, plain compute)
# =============================================================================
# A Python data-science workspace that runs in a Kubernetes pod on the EKS
# cluster. This is plain compute for the Coder Agents path: the AI runs
# server-side on the control plane, so this template carries NO AI Gateway
# wiring, NO LLM keys, and NO agent harness. It provisions Python tooling and
# JupyterLab (exposed as a subdomain app) plus code-server, and requires the
# in-boundary GitLab login for git.
#
...
#
# enterprise-base is Ubuntu noble (ships Python 3), runs as user `coder`
# (uid 1000), ships git/curl/sudo, and grants passwordless sudo so the startup
# script can install JupyterLab and Python tooling. It is the only workspace
# base mirrored today. A pre-baked data-science image (JupyterLab and common
# libraries already installed) is the recommended end state; see the WS-25
...
resource "coder_agent" "main" {
  ...
  # Best-effort data-science tooling. enterprise-base grants the coder user
  # passwordless sudo. Every step is tolerant so a transient failure never
  # fails the workspace build. JupyterLab is launched bound to localhost and
  # proxied by the coder_app below (token and password disabled because Coder
  # handles auth at the app boundary). For a fully air-gapped demo, bake these
  # into a custom workspace image (see WS-25 handoff) and the install becomes a
  # no-op while the launch still works.
  startup_script = <<-EOT
    #!/bin/bash
    set +e
    touch ~/.bashrc
    ...
    if command -v apt-get >/dev/null 2>&1; then
      echo "=== Installing Python data tooling (best-effort) ==="
    ...
    if command -v jupyter-lab >/dev/null 2>&1 || command -v jupyter >/dev/null 2>&1; then
      echo "=== Launching JupyterLab on 127.0.0.1:8888 ==="
      nohup jupyter-lab \
        --no-browser \
...
# JupyterLab served on a subdomain. Coder proxies the owner session to the
# localhost server started by the agent (Jupyter's own token/password are
# disabled because Coder enforces auth at the app boundary).
