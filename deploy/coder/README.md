...
|---|---|
| Dashboard host `dev.usgov.coderdemo.io` | `coder.ingress.host` + `CODER_ACCESS_URL` |
| Workspace-app wildcard `*.usgov.coderdemo.io` | `coder.ingress.wildcardHost` + `CODER_WILDCARD_ACCESS_URL` (single-level, matches the one ACM cert) |
