...
# read from AWS Secrets Manager (usgov-coderdemo/rds/master).
#
# Requirements: kubectl (KUBECONFIG set), aws CLI (profile with read access to
# usgov-coderdemo/rds/master), and a psql reachable to the RDS endpoint. RDS is
...
RDS_MASTER_SECRET="${RDS_MASTER_SECRET:-usgov-coderdemo/rds/master}"
...
kubectl -n "$NS_MONITORING" label secret "$SECRET_NAME" \
  app.kubernetes.io/part-of=usgov-coderdemo \
